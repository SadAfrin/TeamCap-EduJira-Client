    ...
    async function loadData() {
      try {
        ...
        const [statsRes, noticesRes, assignRes, routineRes] = await Promise.all(
          [
            apiGet(
              `/api/stats/student-portal?email=${encodeURIComponent(user?.email || "")}&name=${encodeURIComponent(user?.name || "")}`,
            ),
            apiGet(`/api/notices?role=student&limit=3`),
            apiGet(
              `/api/assignments?className=Class 8&studentId=${encodeURIComponent(user?.id || "STD-801")}`,
            ),
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-300 max-w-xl">
              Stay ahead in your studies. Check today's routine, monitor
              attendance, complete assignments, and ask the 24/7 AI tutor.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            <span
              className={`text-xs font-semibold px-2 py-0.5 rounded-md ${attendancePercent >= 80 ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}
            >
        ...
        <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs">
          ...
          <p className="mt-2 text-xs text-slate-500">
            Mid-Term Term Examination Average: 91.5%
          </p>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-slate-200/90 bg-white p-6 shadow-xs">
          ...
          <div className="mt-5 space-y-3">
            {(routine.length > 0
              ...
              <div
                key={idx}
                className="flex items-center justify-between rounded-xl border border-slate-200/80 bg-slate-50/60 p-3.5 hover:bg-slate-50 transition-colors"
              >
                <div>
                  <div className="flex items-center gap-2">
