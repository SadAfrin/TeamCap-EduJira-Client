  ...
  async function fetchStudents() {
    try {
      ...
      const res = await apiGet(
        `/api/students?className=${selectedClass}&section=${selectedSection}&search=${encodeURIComponent(searchQuery)}`,
      );
      if (res.success) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/80 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            Add, edit, assign class & sections, and manage active student
            records.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="mt-1 text-xs text-slate-500 max-w-sm mx-auto">
              No students match the selected class, section, or search criteria.
              Click "Add New Student" to enroll one.
            </p>
          </div>
        ) : (
          ...
          <div className="relative w-full max-w-2xl rounded-2xl bg-white p-6 sm:p-8 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h2 className="text-lg font-bold text-slate-900">
                {editingStudent
                  ? `Edit Student — ${editingStudent.studentId}`
                  : "Enroll New Student"}
              </h2>
              <button
