...
const PERMISSION_OPTIONS = [
  ...
  { id: "manage_students", label: "Manage Students & Admissions" },
  { id: "manage_teachers", label: "Manage Faculty & Schedules" },
  { id: "academic_settings", label: "Academic & Exam Settings" },
  { id: "reports", label: "View Analytics & Reports" },
];
...
export default function AdminManagementPage() {
  ...
  async function fetchAdmins() {
    try {
      setLoading(true);
      const res = await apiGet(`/api/admins?search=${encodeURIComponent(searchQuery)}`);
      if (res.success) {
        setAdmins(res.data);
      }
    } catch (err: any) {
  ...
  function handleOpenEditModal(admin: Admin) {
  ...
  function togglePermission(permId: string) {
    ...
    if (list.includes(permId)) {
      setFormData({ ...formData, permissions: list.filter((p) => p !== permId) });
    } else {
      setFormData({ ...formData, permissions: [...list, permId] });
    }
  }
  ...
  async function handleSaveAdmin(e: React.FormEvent) {
    ...
    if (!formData.adminId || !formData.name || !formData.email) {
      toast.error("Please fill in Admin ID, Name, and Email");
      return;
    }
    ...
    try {
      setSubmitting(true);
      if (editingAdmin) {
        const id = editingAdmin._id || editingAdmin.adminId;
        const res = await apiPut(`/api/admins/${id}`, formData);
        if (res.success) {
        ...
        const res = await apiPost("/api/admins", formData);
        if (res.success) {
  ...
  async function handleDeleteAdmin(id: string) {
    try {
      const res = await apiDelete(`/api/admins/${id}`);
      if (res.success) {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/80 pb-5">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Administrator Accounts & Roles</h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            Manage administrative staff, permissions, and institutional role privileges.
          </p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-xs">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search admins by name, ID, email, or designation..."
            value={searchQuery}
      ...
      {loading ? (
        <div className="flex h-64 items-center justify-center">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-xs text-slate-400">Loading administrators...</p>
          </div>
        </div>
      ) : filteredAdmins.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
          <h3 className="text-sm font-bold text-slate-800">No Administrators Found</h3>
          <p className="mt-1 text-xs text-slate-500">Add an administrator account to get started.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredAdmins.map((admin) => (
            <div
              key={admin.adminId}
              className="flex flex-col justify-between rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs transition-all hover:shadow-md hover:border-purple-200"
            >
              <div>
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-purple-50 text-purple-700 font-bold text-base border border-purple-100">
                      {admin.name.charAt(0)}
                    </div>
                    <div>
                      <h3 className="font-bold text-slate-900 leading-tight">{admin.name}</h3>
                      <p className="text-xs font-semibold text-purple-600">{admin.designation || "Administrator"}</p>
                    </div>
                  </div>
                ...
                <div className="mt-4 space-y-1.5 text-xs text-slate-600">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono font-bold text-slate-700">{admin.adminId}</span>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="mt-4 border-t border-slate-100 pt-3">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Privileges</p>
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {admin.permissions && admin.permissions.length > 0 ? (
                      admin.permissions.map((p) => (
                        <span key={p} className="rounded-md bg-purple-50 px-2 py-0.5 text-[11px] font-semibold text-purple-700 border border-purple-100">
                          {p === "all" ? "⚡ Full SuperAdmin" : p}
                        </span>
                      ))
                    ) : (
              ...
              <div className="mt-5 flex items-center justify-end gap-2 border-t border-slate-100 pt-3">
                ...
                <button
                  onClick={() => setDeleteConfirmId(admin.adminId)}
          ...
          <div className="relative w-full max-w-xl rounded-2xl bg-white p-6 sm:p-8 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h2 className="text-lg font-bold text-slate-900">
                {editingAdmin ? `Edit Admin Account — ${editingAdmin.name}` : "Create Admin Account"}
              </h2>
              <button
            ...
            <form onSubmit={handleSaveAdmin} className="mt-5 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase text-slate-600 mb-1">Admin ID *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. admin@edujira.edu"
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2 text-sm text-slate-900 focus:border-purple-500 focus:bg-white focus:outline-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. +880 1800-223344"
                    className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2 text-sm text-slate-900 focus:border-purple-500 focus:bg-white focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Academic Dean & IT Director"
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2 text-sm text-slate-900 focus:border-purple-500 focus:bg-white focus:outline-none"
                />
              </div>
              ...
              <div>
                ...
                <div className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
                  {PERMISSION_OPTIONS.map((perm) => {
                    ...
                    return (
                      <label key={perm.id} className="flex items-center gap-2.5 text-xs text-slate-700 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={isChecked}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <p className="mt-2 text-xs text-slate-500">
              Are you sure you want to remove administrator <span className="font-bold text-slate-800">{deleteConfirmId}</span>?
            </p>
